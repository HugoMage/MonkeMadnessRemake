...
public class Tamarin extends TamableAnimal {
    ...
    public Tamarin(EntityType<? extends TamableAnimal> type, Level worldIn) {
        super(type, worldIn);
    ...
    private static final EntityDataAccessor<Integer> DATA_VARIANT_ID = SynchedEntityData.defineId(Tamarin.class, EntityDataSerializers.INT);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(7, new Tamarin.FindItemsGoal());
        this.goalSelector.addGoal(2, new SitWhenOrderedToGoal(this));
        this.goalSelector.addGoal(2, new FollowOwnerGoal(this, 1.0D, 5.0F, 1.0F, true));
        this.goalSelector.addGoal(10, new Tamarin.TarsierEatBerriesGoal(1.2F, 12, 2));
    }
    @Override
    ...
    public class TarsierEatBerriesGoal extends MoveToBlockGoal {
        ...
        public TarsierEatBerriesGoal(double p_28675_, int p_28676_, int p_28677_) {
            super(Tamarin.this, p_28675_, p_28676_, p_28677_);
        }
        ...
        public void tick() {
            ...
            } else if (!this.isReachedTarget() && Tamarin.this.random.nextFloat() < 0.05F) {
                Tamarin.this.playSound(SoundEvents.FOX_SNIFF, 1.0F, 1.0F);
            }
            super.tick();
        }
        protected void onReachedTarget() {
            if (net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(Tamarin.this.level, Tamarin.this)) {
                BlockState blockstate = Tamarin.this.level.getBlockState(this.blockPos);
        ...
        private void pickGlowBerry(BlockState p_148927_) {
            CaveVines.use(p_148927_, Tamarin.this.level, this.blockPos);
        }
        public boolean canUse() {
            return !Tamarin.this.isSleeping() && super.canUse();
        }
        public void start() {
            this.ticksWaited = 0;
            super.start();
    ...
    class FindItemsGoal extends Goal {
        ...
        public boolean canUse() {
            if (!Tamarin.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty()) {
                return false;
            } else if (Tamarin.this.getTarget() == null && Tamarin.this.getLastHurtByMob() == null) {
                if (!Tamarin.this.canMove()) {
                    return false;
                } else if (Tamarin.this.getRandom().nextInt(10) != 0) {
                    return false;
                } else {
                    List<ItemEntity> list = Tamarin.this.level.getEntitiesOfClass(ItemEntity.class, Tamarin.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Tamarin.TRUSTED_TARGET_SELECTOR);
                    return !list.isEmpty() && Tamarin.this.getItemBySlot(EquipmentSlot.MAINHAND).isEmpty();
                }
            } else {
                return false;
            }
        ...
        public void tick() {
            List<ItemEntity> list = Tamarin.this.level.getEntitiesOfClass(ItemEntity.class, Tamarin.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Tamarin.TRUSTED_TARGET_SELECTOR);
            ItemStack itemstack = Tamarin.this.getItemBySlot(EquipmentSlot.MAINHAND);
            if (itemstack.isEmpty() && !list.isEmpty()) {
                Tamarin.this.getNavigation().moveTo(list.get(0), (double)1.2F);
            }
        ...
        public void start() {
            List<ItemEntity> list = Tamarin.this.level.getEntitiesOfClass(ItemEntity.class, Tamarin.this.getBoundingBox().inflate(8.0D, 8.0D, 8.0D), Tamarin.TRUSTED_TARGET_SELECTOR);
            if (!list.isEmpty()) {
                Tamarin.this.getNavigation().moveTo(list.get(0), 1.2F);
            }
